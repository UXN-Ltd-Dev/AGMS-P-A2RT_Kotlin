...
def getGlucoseList(current_list, event_list):
    ...
    current_list_py = [current_list.get(i) for i in range(current_list.size())]
    event_list_py = [event_list.get(i) for i in range(event_list.size())]

    print("current_list : ", current_list_py)
    print("event_list : ", event_list_py)

    if not current_list_py:
        print("current_list is empty. Function will return.")
        return

    if not event_list_py:
        print("event_list is empty. Function will return.")
        return

    # data = request.get_json()
    current = current_list_py
    event = event_list_py
    ...
    parameterData = {
        ...
        'intercept': [3.899827, 3.899827, 3.899827, 3.899827, 3.899827, 3.899827, 3.899827,
                      3.899827, 3.899827, 3.899827, 3.899827, 3.899827, 3.899827, 3.899827,
                      3.899827, 3.899827],
    ...
    glucoseData = {
        "experiment_date": [item.getCreatedAt() for item in current_list_py],
        "value_current": [item.getWeCurrent() for item in current_list_py],
        "value_ae": [item.getAeCurrent() for item in current_list_py]
    }
    ...
    eventData = {
        "event_time": [item.getCreatedAt() for item in event_list_py],
        "event_value": [item.getValue() for item in event_list_py]
    }
    ...
    for idx, row in df.iterrows():
        ...
        if idx + 1 < len(df):
            ...
            if event_base_count < len(eventDF):
                if row['ts'] <= eventDF.iloc[event_base_count]['ts'] and \
                        eventDF.iloc[event_base_count]['ts'] <= next_row_ts:
                    print('ok')
                    print(event_base_count)
                    print(eventDF.iloc[event_base_count]['ts'])
                    print(eventDF.iloc[event_base_count]['event_value'])
                    onepoint_calibration_wrapper(eventDF.iloc[event_base_count]['ts'] - base_second,
                                                 eventDF.iloc[event_base_count]['event_value'])
        ...
        jsonFormatData = result.to_json(orient='records')

        # result_list = result.to_dict(orient='records')

    print("jsonFormatData : ", jsonFormatData)
